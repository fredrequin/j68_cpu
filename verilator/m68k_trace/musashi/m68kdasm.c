...
static char* make_signed_hex_str_8(uint val)
{
	...
	if(val == 0x80)
		...
		sprintf(str, "-$%02X", (0-val) & 0x7f);
	else
		sprintf(str, "$%02X", val & 0x7f);
...
static char* make_signed_hex_str_16(uint val)
{
	...
	if(val == 0x8000)
		...
		sprintf(str, "-$%04X", (0-val) & 0x7fff);
	else
		sprintf(str, "$%04X", val & 0x7fff);
...
static char* make_signed_hex_str_32(uint val)
{
	...
	if(val == 0x80000000)
		...
		sprintf(str, "-$%08X", (0-val) & 0x7fffffff);
	else
		sprintf(str, "$%08X", val & 0x7fffffff);
...
static char* get_imm_str_u(uint size)
{
	...
	if(size == 0)
		...
		sprintf(str, "#$%08X", read_imm_32() & 0xffffffff);
	return str;
}
...
static char* get_ea_mode_str(uint instruction, uint size)
{
	...
	switch(instruction & 0x3f)
	{
		...
		/* absolute short address */
			sprintf(mode, "$%04X.w", read_imm_16());
			break;
		case 0x39:
		/* absolute long address */
			sprintf(mode, "$%08X.l", read_imm_32());
			break;
		case 0x3a:
		/* program counter with displacement */
			temp_value = read_imm_16();
			sprintf(mode, "$%08X(PC)", (make_int_16(temp_value) + g_cpu_pc-2) & 0xffffffff);
			break;
		case 0x3b:
		/* program counter with index */
			...
			if(EXT_8BIT_DISPLACEMENT(extension) == 0)
				...
				sprintf(mode, "$%08X(PC,%c%d.%c", (make_int_8(extension) + g_cpu_pc-2) & 0xffffffff, EXT_INDEX_AR(extension) ? 'A' : 'D', EXT_INDEX_REGISTER(extension), EXT_INDEX_LONG(extension) ? 'l' : 'w');
			if(EXT_INDEX_SCALE(extension))
				sprintf(mode+strlen(mode), "*%d", 1 << EXT_INDEX_SCALE(extension));
			strcat(mode, ")");
			break;
...
{
	sprintf(g_dasm_str, "andi.b  %s,CCR", get_imm_str_u8());
}

static void d68000_andi_to_sr(void)
{
	sprintf(g_dasm_str, "andi.w  %s,SR", get_imm_str_u16());
}
...
static void d68000_bcc_8(void)
{
	...
	sprintf(g_dasm_str, "b%-2s.b   $%08X", g_cc[(g_cpu_ir>>8)&0xf], temp_pc + make_int_8(g_cpu_ir));
}

static void d68000_bcc_16(void)
{
	uint temp_pc = g_cpu_pc;
	sprintf(g_dasm_str, "b%-2s.w   $%08X", g_cc[(g_cpu_ir>>8)&0xf], temp_pc + make_int_16(read_imm_16()));
}
...
static void d68020_bcc_32(void)
{
	...
	sprintf(g_dasm_str, "b%-2s.l   $%08X; (2+)", g_cc[(g_cpu_ir>>8)&0xf], temp_pc + read_imm_32());
}
...
{
	uint temp_pc = g_cpu_pc;
	sprintf(g_dasm_str, "bra.b   $%08X", temp_pc + make_int_8(g_cpu_ir));
}

static void d68000_bra_16(void)
{
	uint temp_pc = g_cpu_pc;
	sprintf(g_dasm_str, "bra.w   $%08X", temp_pc + make_int_16(read_imm_16()));
}
...
static void d68020_bra_32(void)
{
	uint temp_pc = g_cpu_pc;
	LIMIT_CPU_TYPES(M68020_PLUS);
	sprintf(g_dasm_str, "bra.l   $%08X; (2+)", temp_pc + read_imm_32());
}
...
{
	uint temp_pc = g_cpu_pc;
	sprintf(g_dasm_str, "bsr.b   $%08X", temp_pc + make_int_8(g_cpu_ir));
}

static void d68000_bsr_16(void)
{
	uint temp_pc = g_cpu_pc;
	sprintf(g_dasm_str, "bsr.w   $%08X", temp_pc + make_int_16(read_imm_16()));
}
...
static void d68020_bsr_32(void)
{
	uint temp_pc = g_cpu_pc;
	LIMIT_CPU_TYPES(M68020_PLUS);
	sprintf(g_dasm_str, "bsr.l   $%08X; (2+)", temp_pc + peek_imm_32());
}
...
{
	uint temp_pc = g_cpu_pc;
	sprintf(g_dasm_str, "dbra    D%d,$%08X", g_cpu_ir & 7, temp_pc + make_int_16(read_imm_16()));
}

static void d68000_dbcc(void)
{
	uint temp_pc = g_cpu_pc;
	sprintf(g_dasm_str, "db%-2s    D%d,$%08X", g_cc[(g_cpu_ir>>8)&0xf], g_cpu_ir & 7, temp_pc + make_int_16(read_imm_16()));
}
...
{
	sprintf(g_dasm_str, "eori.b  %s,CCR", get_imm_str_u8());
}

static void d68000_eori_to_sr(void)
{
	sprintf(g_dasm_str, "eori.w  %s,SR", get_imm_str_u16());
}
...
{
	sprintf(g_dasm_str, "move.b  %s,CCR", get_ea_mode_str_8(g_cpu_ir));
}

static void d68010_move_fr_ccr(void)
{
	LIMIT_CPU_TYPES(M68010_PLUS);
	sprintf(g_dasm_str, "move.b  CCR,%s; (1+)", get_ea_mode_str_8(g_cpu_ir));
}

static void d68000_move_fr_sr(void)
{
	sprintf(g_dasm_str, "move.w  SR,%s", get_ea_mode_str_16(g_cpu_ir));
}

static void d68000_move_to_sr(void)
{
	sprintf(g_dasm_str, "move.w  %s,SR", get_ea_mode_str_16(g_cpu_ir));
}

static void d68000_move_fr_usp(void)
{
	sprintf(g_dasm_str, "move.l  USP,A%d", g_cpu_ir&7);
}

static void d68000_move_to_usp(void)
{
	sprintf(g_dasm_str, "move.l  A%d,USP", g_cpu_ir&7);
}
...
{
	sprintf(g_dasm_str, "movep.w D%d,$%04X(A%d)", (g_cpu_ir>>9)&7, read_imm_16(), g_cpu_ir&7);
}

static void d68000_movep_re_32(void)
{
	sprintf(g_dasm_str, "movep.l D%d,$%04X(A%d)", (g_cpu_ir>>9)&7, read_imm_16(), g_cpu_ir&7);
}

static void d68000_movep_er_16(void)
{
	sprintf(g_dasm_str, "movep.w $%04X(A%d),D%d", read_imm_16(), g_cpu_ir&7, (g_cpu_ir>>9)&7);
}

static void d68000_movep_er_32(void)
{
	sprintf(g_dasm_str, "movep.l $%04X(A%d),D%d", read_imm_16(), g_cpu_ir&7, (g_cpu_ir>>9)&7);
}
...
{
	sprintf(g_dasm_str, "ori.b   %s,CCR", get_imm_str_u8());
}

static void d68000_ori_to_sr(void)
{
	sprintf(g_dasm_str, "ori.w   %s,SR", get_imm_str_u16());
}
